...
process merge {
    ...
    input:
        set val(sample), path(vcf_path1), path(vcf_path2), path(vcf_path3) from vartype_out1
...
process snpfilter {
    ...
    input:
        set val(sample), path(vcf_path1) from spread_out
        set val(sample), path(vcf_path2) from merge_out2
        set val(sample), path(bam_path1) from postal_out5
    ...
    script:
        """
        python $baseDir/snpreport1-4.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample}
        """
}