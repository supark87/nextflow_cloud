...
process processAlignments {
    ...
    script:
        """
        java -jar picard.jar AddOrReplaceReadGroups -I ${sam_path} -O ${sample}_SR.bam -SORT_ORDER coordinate --CREATE_INDEX true \\
...
process summarize {
    container 'tools:latest'
    publishDir "$params.output.folder/summary/${sample}", mode : "copy"

    input:
        set val(sample), path(vcf_path) from spread_out

    output:
        tuple val(sample), path("fixedPOS${vcf_path}") into summary_out

    script:
    """
    python $baseDir/snpfilter1.py -r ref/pfalciparum/mdr.fa -b ref/pfalciparum/mdr.bed -v ref/pfalciparum/Reportable_SNPs.csv -o test1
